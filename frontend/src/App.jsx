    ...
    <main className="h-screen w-full bg-[#09090b] text-white flex overflow-hidden font-sans">
      ...
      <div className="w-[240px] border-r border-gray-800 flex flex-col bg-[#18181b] shrink-0">
        <div className="p-3 border-b border-gray-800 flex items-center justify-between">
          ...
          <button 
            onClick={() => { if(confirm('Her şey silinecek?')) store.resetWorkspace(); }}
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {store.files.map((file) => (
            <div 
              ...
              className={`flex items-center justify-between px-3 py-2 cursor-pointer rounded transition-colors text-xs ${
            ...
            >
              ...
              <button onClick={(e)=>{e.stopPropagation(); store.deleteFile(file.id)}} className="hover:text-red-400"><Trash2 size={12}/></button>
            </div>
          ))}
          {store.files.length === 0 && <div className="text-center mt-10 text-gray-600 text-xs">Dosya yok.</div>}
        </div>
      </div>
      ...
      <div className={`flex flex-col bg-[#1e1e1e] border-r border-gray-800 transition-all duration-300 h-full ${showEditor ? 'w-[400px] shrink-0' : 'flex-1'}`}>
        <div className="h-10 border-b border-gray-800 px-4 flex justify-between items-center bg-[#1e1e1e] shrink-0">
          ...
          <button onClick={() => setShowEditor(!showEditor)} className="text-gray-400 hover:text-white">
      ...
      {showEditor && (
        <div className="flex-1 bg-[#1e1e1e] flex flex-col min-w-0 h-full">
           <div className="h-10 bg-[#1e1e1e] border-b border-gray-800 flex items-center px-4 text-xs text-blue-400">
